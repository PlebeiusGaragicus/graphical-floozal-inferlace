...
class Dialog(Widget):
    ...
    def handle_mouse(self, x, y):
        ...
        if self.inside(x, y):
            ...
            # OTHERWISE SELF.FOCUS_IDX IS CHANGED EVEN IF A NON-FOCUSABLE ITEM IS CLICKED.
            # THEN... ONCE YOU TAB IT RUNS INTO ERRORS
...
class WButton(FocusableWidget):
    ...
    def redraw(self):
        ...
        # RED CANCEL BUTTON!!!
        if self.finish_dialog == ACTION_CANCEL:
            bcolor = C_RED
        else:
            bcolor = C_GREEN
        ...
        if self.disabled:
            ...
            if self.focus:
                self.attr_color(C_B_WHITE, bcolor)
            else:
                self.attr_color(C_BLACK, bcolor)
